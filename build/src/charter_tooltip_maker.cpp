...
CharterTooltipMaker::CharterTooltipMaker(QwtPlot* plot, double searchWidth)
	...
	connect(this, SIGNAL(closestXYPicked(QPointF)),
		this, SLOT(make_point_marker(QPointF)));

}

void CharterTooltipMaker::make_point_marker(QPointF point) {

	QString markerString = "Marker at: " + QString::number(point.x()) + ", " +
		QString::number(point.y());
	QwtText markerLabel(markerString);

	AbstractPlotMarker* marker = new PointPlotMarker(markerLabel, point);
	if(this->plot()) marker->attach(this->plot());

	emit pointMarkerMade(marker);
